...
public class EnrollmentServiceImpl implements EnrollmentService {
    ...
    public Enrollment enrollStudent(EnrollmentDto dto) {
        ...
        if (dto.getPracticalMarks() != null) {
            enrollment.setPracticalMarks(dto.getPracticalMarks());
        }
    ...
    @Override
    @Transactional
    public Enrollment updateAssessment(Long enrollmentId, Double numericGrade, Double attendancePercentage, String labStatus, String labSubmissionDetails) {
        return updateAssessment(enrollmentId, numericGrade, attendancePercentage, labStatus, labSubmissionDetails, null);
    }

    @Override
    @Transactional
    public Enrollment updateAssessment(Long enrollmentId, Double numericGrade, Double attendancePercentage, String labStatus, String labSubmissionDetails, Double practicalMarks) {
        Enrollment enrollment = getEnrollmentById(enrollmentId);
        if (numericGrade != null) {
            enrollment.updateGrade(numericGrade);
            enrollment.setStatus("COMPLETED");
        ...
        if (labStatus != null && !labStatus.isBlank()) {
            enrollment.setLabStatus(labStatus);
        }
        if (labSubmissionDetails != null && !labSubmissionDetails.isBlank()) {
            enrollment.setLabSubmissionDetails(labSubmissionDetails);
        }
        if (practicalMarks != null) {
            enrollment.setPracticalMarks(practicalMarks);
        }
        return enrollmentRepository.save(enrollment);
    }
