...
public class Student {
    ...
    public double calculateAverageAttendance() {
        ...
        for (Enrollment e : enrollments) {
            if (e.getAttendancePercentage() != null) {
                total += e.getAttendancePercentage();
                count++;
            }
        }
        return count == 0 ? 100.0 : Math.round((total / count) * 10.0) / 10.0;
    }

    public String getPrimaryLabStatus() {
        if (enrollments == null || enrollments.isEmpty()) {
            return "SUBMITTED";
        }
        for (Enrollment e : enrollments) {
            if (e.getLabStatus() != null && !e.getLabStatus().isBlank()) {
                return e.getLabStatus();
            }
        }
        return "SUBMITTED";
    }
